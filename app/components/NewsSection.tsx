...
export default function NewsSection() {
  // Sortare safe + nu crapă dacă date e invalid
    ...
    <section id="noutati" className="scroll-mt-28 border-t border-white/5">
      <div className="mx-auto max-w-6xl px-8 md:px-10 py-16 md:py-18">
        ...
        <p className="mt-2 text-zinc-300/85 max-w-2xl">
          Update-uri, proiecte și materiale media. Fiecare are pagină proprie pentru share (OG).
        </p>

        {items.length === 0 ? (
          <div className="mt-8 rounded-2xl border border-white/10 bg-white/[0.03] p-6 text-zinc-300/85">
            Nu există noutăți încă. Adaugă elemente în <code>app/data/news.ts</code>.
          </div>
        ) : (
          <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            {items.slice(0, 4).map((x) => (
              <Link
                key={x.slug}
                href={`/noutati/${x.slug}`}
                className="group rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden hover:bg-white/[0.06] hover:border-amber-300/40 transition"
              >
                {/* THUMBNAIL - folosim ogImage (la tine e /noutati/din2017.jpg) */}
