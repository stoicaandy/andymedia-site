...
export default function PortfolioCard({ item, priority = false, onOpenGallery }: Props) {
  ...
  return (
    <article className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 md:p-5 backdrop-blur-sm">
      {/* MAIN MEDIA */}
      ...
      {photos.length > 1 && (
        <div className="mt-3 grid grid-cols-6 gap-2">
          {thumbs.shown.map((src, i) => {
            ...
            return (
              <button
                key={src}
                type="button"
                onClick={() => onOpenGallery(gallery, realIndex)}
          ...
          {thumbs.remaining > 0 && (
            <button
              type="button"
              onClick={() => onOpenGallery(gallery, thumbs.shown.length + 1)}
